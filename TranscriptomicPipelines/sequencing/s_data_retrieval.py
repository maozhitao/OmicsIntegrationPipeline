...
class SequencingRetrievalParameters(object):
    def __init__(   self,
...
class SequencingRetrievalResults(object):
    def __init__(self):
...
class SequencingRetrieval(s_module_template.SequencingSubModule):
    ...
    def download_srainfo(self):
        if self.parameters.skip_srainfo_download == False or self.check_existed_srainfo() == False:
            ...
            df_list = []
            for id_ in self.get_s_query_id():
                print(id_)
                handle = Entrez.efetch( id      = id_,
    ...
    def complete_data_independent_metadata(self):
        ...
        for index, row in self.sra_run_info.iterrows():
            metadata.new_sequencing_entry(
                platform_id = row[SequencingRetrievalConstant.SRAINFO_COL_PLATFORM.value],
    ...
    def _remove_run(self, exp, run_id_remove):
        """private helper method for filter_entry"""
    ...
    def filter_entry(   self,
                        platform_id_remove      = [],
                        series_id_remove        = [],
                        experiment_id_remove    = [],
                        run_id_remove           = []):
        for exp in self.mapping_experiment_runs:
            self._remove_run(exp, run_id_remove)
